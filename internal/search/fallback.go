...
func SearchChain(ctx context.Context, sources map[string]Searcher, chain []string, query string, limit int, skipMissing bool) (Result, string, *Error) {
	names := chain
	if skipMissing {
	...
	for _, name := range names {
		...
		result, err := src.Search(ctx, query, limit)
		if err == nil {
			if !IsEmpty(result) {
